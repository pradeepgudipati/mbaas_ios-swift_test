...
public class MbaasAPI {
    ...
    static let sharedInstance: MbaasAPI = {
          return MbaasAPI() // Lazy loading of singleton object.
    }()
...
public class RequestBuilder<T> {
    ...
    required public init(method: String,
        ...
        addHeaders(aHeaders: MbaasAPI.sharedInstance.customHeaders)
        // Add additional headers from the authName
        for singleAuthName in authNames {
             if let authenticator = MbaasAPI.sharedInstance.getAuthenticator(name: singleAuthName) {
        ...
        if (authNames.count == 0 && MbaasAPI.sharedInstance.authenticators.count == 1) {
            if let singleAuthenticator =  MbaasAPI.sharedInstance.authenticators.first?.value {
    ...
    public func addCredential() -> Self {
        self.credential = MbaasAPI.sharedInstance.credential
        return self
    }
}
